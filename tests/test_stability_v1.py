...
def _config() -> dict:
    # The v1 stability block was renamed to ``stability_v1`` when Step 2 v2
    # took over the ``stability`` key in configs/spb_default.yaml. The v1
    # archive code still reads ``config["stability"][...]``, so remap here.
    cfg = load_config("configs/spb_default.yaml")
    if "stability_v1" in cfg:
        cfg = {**cfg, "stability": cfg["stability_v1"]}
    return cfg
